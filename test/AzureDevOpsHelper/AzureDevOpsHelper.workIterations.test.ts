...
test('AzureDevOpsHelper - workIterations', async () => {
    ...
    for (const team of teams.value!.filter(p => p.id !== undefined && p.projectName !== undefined).slice(0, maxNumerOfTests)) {
        ...
        TestHelper.checkValueAndError(workIterations, { organization, project, teamId });
        ...
        for (const workIteration of workIterations.value!.slice(0, maxNumerOfTests)) {
            ...
            TestHelper.checkValueAndError(workIterationResponse, { organization, project, teamId, iterationPath: workIteration.path });
        }
    }
}, 100000);