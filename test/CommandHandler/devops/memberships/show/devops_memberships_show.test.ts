...
test('devops_memberships_show-groups', async () => {
    ...
    const principalNames = groups.filter(p => p.principalName !== undefined).map(p => p.principalName!).map(p => p.toLowerCase()).sort();
...
test('devops_memberships_show-users', async () => {
    ...
    const principalNames = users.filter(p => p.principalName !== undefined).map(p => p.principalName!).map(p => p.toLowerCase()).sort();
...
test('devops_memberships_show-all', async () => {
    ...
    const principalNamesUsers  = (await azureDevOpsHelper.graphUsersListForProjectName (organization, projectName, maxNumberOfTests)).filter(p => p.principalName !== undefined).map(p => p.principalName!).map(p => p.toLowerCase()).sort();
    const principalNamesGroups = (await azureDevOpsHelper.graphGroupsListForProjectName(organization, projectName, maxNumberOfTests)).filter(p => p.principalName !== undefined).map(p => p.principalName!).map(p => p.toLowerCase()).sort();
