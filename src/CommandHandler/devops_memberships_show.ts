...
import { ItemAndOthersEx } from "../ItemAndOthersEx";
...
export class devops_memberships_show {
    ...
    static async handle(tenantId: string, organization: string, projectName: string | undefined, principalName: string, path: string): Promise<void> {
        ...
        const fileUngrouped = `${file}.md`        ; await rm(fileUngrouped, { force: true });
        const fileGrouped   = `${file}-compact.md`; await rm(fileGrouped  , { force: true });
        ...
        if (graphMember?.descriptor === undefined) {
            throw new Error(JSON.stringify({ organization, principalName, graphMember, message: 'Failed to resolve graphMember.descriptor.' }));
        }
        ...
        const itemAndOthersExDown = itemAndOthersDown === undefined ? undefined : new ItemAndOthersEx(itemAndOthersDown);
        const itemAndOthersExUp   = itemAndOthersUp   === undefined ? undefined : new ItemAndOthersEx(itemAndOthersUp  );
        ...
        await Promise.all([
            writeFile(fileUngrouped, markdown),
            AzureDevOpsHelper.isGraphGroup(graphMember) ? writeFile(fileGrouped, markdownEx) : Promise<void>,
        ]);
        ...
        console.log(JSON.stringify({
            ...
            files: {
                markdown: {
                    explicit: fileUngrouped,
                    compact : AzureDevOpsHelper.isGraphGroup(graphMember) ? fileGrouped : undefined
                }
            }
        }, null, 2));
    }
    ...
    private static filter(principalName: string, blackList:string[], project: TeamProjectReference | undefined, items: Array<{ container: GraphMember; member: GraphMember; }>){
        return items
            .filter(p => blackList.find(m => `${p.container.principalName}`.indexOf(m) > 0) === undefined)
