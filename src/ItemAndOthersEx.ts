...
export class ItemAndOthersEx {
    readonly item  : GraphMember;
    readonly users : Array<GraphUser>;
    readonly others: Array<ItemAndOthersEx>;

    constructor(
        itemAndOthers: ItemAndOthers
    ) {
        this.item = itemAndOthers.item;
        this.users = new Array<GraphUser>();
        this.others = new Array<ItemAndOthersEx>();
        ...
        for (const other of itemAndOthers.others) {
            if (AzureDevOpsHelper.isGraphUser(other.item)) {
                ...
                this.others.push(new ItemAndOthersEx(other));
            }
            else {
    ...
    private getFlatDown(items: Array<ItemAndOthersEx>): Array<{ container: GraphMember; users: Array<GraphUser>; } | { container: GraphMember; group: GraphGroup; }> {
        const map = new Map<string, { container: GraphMember; users: Array<GraphUser>; } | { container: GraphMember; group: GraphGroup; }>();
